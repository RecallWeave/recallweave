...
- **Open source** opens `obsidian://open` when the loaded graph carries an
  explicit `vault_name` label claim; otherwise it copies the relative note path
  (Atlas never invents a vault label).
